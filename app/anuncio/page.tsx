...
function AnuncioForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. GUARDAR EL ANUNCIO (Estado inicial: DRAFT)
      const { data: adData, error: adError } = await supabase
        ...
        .insert({
          ...
          category,
          user_id: userId,
          status: 'draft', // Comienza invisible
      ...
      // 3. SUBIR LA FOTO (DIRECTO A SUPABASE - CLIENT SIDE)
      if (photos.length > 0) {
        const file = photos[0];
        const fileExt = file.name.split('.').pop();
        const fileName = `${adData.id}/image_${Date.now()}.${fileExt}`;

        // USO DIRECTO DEL CLIENTE SUPABASE (Evita Error 500 de API)
        ...
        if (uploadError) {
          ...
          setMessage('⚠️ Anuncio guardado, pero la imagen falló. Intenta editarlo.');
        } else {
          // ÉXITO: ACTIVAR PARA EL ADMIN
          await supabase
            .from('ads')
        ...
        // SIN FOTO: ACTIVAR IGUAL (Opcional)
        await supabase
          .from('ads')
          .update({ status: 'pending_verification' })
          .eq('id', adData.id);
        setMessage('✅ Anuncio enviado (Sin foto).');
      }
